...
import { gqlResult, Edge } from './gql'
...
export async function loadDataArray(searchTerm: string, edges: Edge[]) {
  ...
  for (const { node } of edges) {
    ...
    for (const topic of groupInfo.topics) {
      await (
        await remult.repo(Topic).findFirst(
          {
            groupId: group.id,
            topicId: topic.id
          },
    ...
    for (const { node } of groupInfo.unifiedEvents.edges) {
      await (
        await remult.repo(Event).findFirst(
          {
            groupId: group.id,
            eventId: node.id
          },
